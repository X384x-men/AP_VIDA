...
import { GlobalVariable, SolicitudVariable, UsuarioAcceso} from 'src/app/core/static/variables/url/URLImages';
...
@Component({
  ...
  templateUrl: './dashboard-analistas.component.html',
  styleUrls: ['./dashboard-analistas.component.css']
})
export class DashboardAnalistasComponent implements OnInit {
  ...
  getHistoricos = ( idSolicitud ) => {
    console.log(idSolicitud);
    this.subResourceService.read(SolicitudVariable.GET_EVENTOS_SOLIITUDES, {
      idSolicitud: idSolicitud,
    }
  ).subscribe( (data) => {
      this.eventoHistorico = data.length !== 0 ? data : 'No hay registro';
  }, error=>{
    swal(error, '', 'error');
  });
  }

  reAsignrSolicitud = ( forms : NgForm ) => {
    if( this.idSolciitudes && forms.form.value ){
      this.SolicitudesS.reAsignarSolicitud( this.idSolciitudes, forms.form.value.idAnalista ).subscribe( (resp : any) => {
        swal( 'Se ha realizado la asignación satisfactoriamente', '', 'success' ).then ( () => {
          this.loading = true;
          this.getSolicitudes()
        });
      }, error => {
  ...
  filtrarAnalistas = () => {
    this.subResourceService.read(UsuarioAcceso.GET_LIST_EMPLEADOS_SEARCH, {
      ...
      data.forEach(element => {
        if (element.tipoAnalista === 1) {
          this.analistas.push(element)
        }
      });
  },(error) => {
    console.log({error});
