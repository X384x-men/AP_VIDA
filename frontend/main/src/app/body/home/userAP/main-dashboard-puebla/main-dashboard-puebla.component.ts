...
@Component({
  selector: 'app-main-dashboard-puebla',
  templateUrl: './main-dashboard-puebla.component.html',
  styleUrls: ['./main-dashboard-puebla.component.css']
})
export class MainDashboardPueblaComponent {
  ...
  ngOnInit() {
    this.authencationService.validacionUserPuebla();
    this.userApp = JSON.parse(localStorage.getItem("currentUserPuebla"));
    this.refreshList();
  }
  ...
  verDetalleSolicitud(item){
    this.router.navigate(['/angular/form-edit-solicitudes'], {queryParams: {solicitud: item.idSolicitud, categoria: 'puebla', opt: 2}});
  }
  ...
  downloadSolicitudBatch = () => {
    ...
    this.excelService.exportAsExcelFile(dataLayout, 'SolicitudesTotalesPuebla' + this.today);
